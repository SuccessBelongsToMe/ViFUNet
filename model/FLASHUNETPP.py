...
class ResNet34UnetPlusFLASH(nn.Module):
    def __init__(self,
                 num_channels=1,
                 num_class=1,
                 is_deconv=False,
        ...
        self.flash_vit1 = FLASHTransformer(in_channel=64,path_size=1,emb_size=128, dim=128,depth=12,group_size=32,query_key_dim=128,img_size=16)
        self.flash_vit2 = FLASHTransformer(in_channel=128,path_size=1,emb_size=256, dim=256,depth=12,group_size=64,query_key_dim=128,img_size=8)
        self.flash_vit3 = FLASHTransformer(in_channel=256,path_size=1,emb_size=512, dim=512,depth=12,group_size=128,query_key_dim=128,img_size=4)
        self.flash_vit4 = FLASHTransformer(in_channel=512,path_size=1,emb_size=1024, dim=1024,depth=12,group_size=512,query_key_dim=128,img_size=2)
    ...
    def forward(self, x):
        ...
        e1 = self.flash_vit1(x)  #64
        print(e1.shape)
        e2 = self.flash_vit2(e1)
        print(e2.shape)  #128
        e3 = self.flash_vit3(e2)  #256
        print(e3.shape)
        e4 = self.flash_vit4(e3)  #512
        print(e4.shape)
...
if __name__ == '__main__':
    model = ResNet34UnetPlusFLASH( num_channels=1,
                 num_class=2,
                 is_deconv=False,
                 decoder_kernel_size=3)
